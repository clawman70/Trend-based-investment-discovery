...
**Not live-verified:** no Anthropic *or* Google API key was available in this environment (the Gemini key present before Phase 2 was inadvertently removed when Research first moved to Claude, and its value was never visible to Claude Code to restore — Jeff needs to re-add `GOOGLE_GENAI_API_KEY` to `.env.local`). `tsc`/`eslint`/`vitest`/`next build` all pass and unit tests cover both providers' citation-reconciliation logic directly, but neither provider's actual API calls have been exercised for real — Claude's structured-output + web_search combination was never used in the end (Research moved to Gemini before that combination was tested), and Gemini's structured-output + googleSearch-tool combination (the pre-existing pattern from before this rework) has also never been live-verified in this project. First `npm run dev` session with both keys should treat Discover and Research as a smoke test.
